...
-----------------------------  Condition  ---------------------------------
  ...
  behavior HoareCondition
      ...
      method Wait (mutex: ptr to HoareMutex, next: ptr to HoareMutex, next_count: ptr to int)
          var
            oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)
      ...
      method Signal (next: ptr to HoareMutex, next_count: ptr to int)
          var
            oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method GetANewThread () returns ptr to Thread
        ...
        --  threadManagerLock.Lock ()
        --  if freeList.IsEmpty ()
        --    aThreadBecameFree.Wait (&threadManagerLock, &next, &next_count)
        --  endIf
        --  th = freeList.Remove ()
        --  th.status = JUST_CREATED
      ...
      method FreeThread (th: ptr to Thread)
        ...
        ------------------------------Hoare-----------------------------
        --  aThreadBecameFree.Signal (&next, &next_count)
        ----------------------------------------------------------------
...
-----------------------------  StartUserProcess  --------------------------
  ...
  function StartUserProcess ()
    ...
    if file == null
      FatalError ("Failure in openning")
    endIf

    newAddrSpace.Init ()
    initPC = file.LoadExecutable(&newAddrSpace)
    if initPC < 0
      FatalError ("Failure in LoadExecutable")
    endIf
    fileManager.Close (file)

    frameManager.ReturnAllFrames (&p.addrSpace)
    p.addrSpace = newAddrSpace

    initUserStackTop = PAGE_SIZE * newAddrSpace.numberOfPages
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        ...
        for i = 0 to numFramesNeeded - 1
          idx = framesInUse.FindZeroAndSet ()
          addr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + idx * PAGE_SIZE
          aPageTable.SetFrameAddr (i, addr)
        endFor
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
        ...
        for i = 0 to aPageTable.numberOfPages - 1
          ...
          base = (addr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE
          framesInUse.ClearBit (base)
        endFor
