...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
          stat = 0                                -- this is the lock, 0 means unlocked and 1 means lock
          ownMutex = null                         -- this is defined because of the method IsHeldByCurrentThread () we didn't have it in the class code 
          waitingThreads = new List [Thread]
        endMethod
      ...
      method Lock ()
          var
            oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)
          if stat == 0                            -- is not locked before
            stat = 1
            ownMutex = currentThread
          else
            waitingThreads.AddToEnd (currentThread)     -- locked before so we should wait
            currentThread.Sleep ()
          endIf
          oldIntStat = SetInterruptsTo (oldIntStat) 
        endMethod
      ...
      method Unlock ()
          var
            ...
            t: ptr to Thread
          oldIntStat = SetInterruptsTo (DISABLED)
          if self.IsHeldByCurrentThread ()        -- unlock
            stat = 0
            if !waitingThreads.IsEmpty ()         -- remove one from waiting list
              t = waitingThreads.Remove ()
              t.status = READY
              readyList.AddToEnd (t)
              ownMutex = t
